...
* The **config-check** job validates incoming patches (linting, syntax, etc).
  ...
  and just before it is merged). The job's playbook can be found `here: check.yaml
...
* Once merged, the **config-update** job is executed to apply the new
  configuration to Software Factory. The job's playbook can be found `here: update.yaml
...
can be used when writing jobs; the full list and descriptions are available `on Zuul's online documentation <https://zuul-ci.org/docs/zuul/user/jobs.html#zuul-variables>`_.
