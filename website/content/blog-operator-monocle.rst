...
Resources' names must be unique in a single ``namespace``.
...
called then ``rollOutWhenApiSecretsChange`` function checks if the
resources version changed. In the case of a change (meaning that the
administrator changed one of the Monocle secrets) we do an ``Update`` of
the annotation to store the new ``apiSecretsVersion`` value.
...
-  the ClusterRole ``monocle-operator-manager-role``. This role defines
   `authorizations`_ needed by the ``controller`` to act on the
