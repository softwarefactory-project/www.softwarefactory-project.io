...
You can find the ``monocle-operator`` on `github page`_.
...
exists we want to provide a working Monocle deployment with the
database, the api, and the crawler.
...
Monocle's administrator changes the secrets we used an annotation (we
...
A Monocle instance can be reclaim to the operator by applying the Sample
resource:
...
The generated image can be found on the your locally and then can be
push to the image registry via:
...
.. _github page: https://github.com/change-metrics/monocle-operator/tree/813eb65df2da2249a5f2f0dd348ac4a3b6f11f0c
