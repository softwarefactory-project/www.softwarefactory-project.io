...
Capabilities of an operator are splitted into `phases`_ as described in
...
to detect changes in its configuration file and reload itself the
configuration.
...
``api/v1alpha1/monocle_types.go``\ `monocle-types`_) so we need to get
the instance's ``spec`` to gather all information about the expected
state.
...
start developing an Operator, `kubebuiler`_, `operator SDK`_, then which
stack to use for the operator's development `helm`_, `ansible`_ or
`go`_, and at the same time learning new things and finding out how
Kubernetes works in more detail every day. It was a great and
chanllenging oportunity to learn this big new world of Kubernetes, and
we hope to have made the right choices for the continuity of Monocle
Operator. Stay tuned for the next posts, where we will continue in this
exiting new world of Operators. Fell free to talk to us.
