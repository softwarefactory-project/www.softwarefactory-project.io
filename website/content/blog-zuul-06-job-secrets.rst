...
post-review pipeline such as periodic or post.
...
.. code-block:: bash
   ...
   curl http://sftests.com/zuul/api/tenant/local/key/config.pub
...
project so that it is published to pypi on release.
...
Also make sure the following packages are installed on the Zuul executor:
