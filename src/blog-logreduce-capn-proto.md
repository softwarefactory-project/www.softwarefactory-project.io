...
- [Flatbuffers][flatbuffers], introduced by Google in 2014, designed for games with an opt-in costly data validation.

Cap'n Proto and Flatbuffers are modern formats designed for performance-critical applications.
They both enable access to the data without parsing/unpacking, using a process known as zero-copy serialization.
Flatbuffers was originally created for games development and data validation is an opt-in costly process.
Therefor I decided to use Cap'n Proto for logreduce as discussed in the next sections.
...
because it now expects that Zuul builds have a project field.
...
code for [various languages](https://capnproto.org/otherlang.html).
Use the following build instruction for rust:
...
The decoder/encoder benchmark loads the full report struct.
Cap'n Proto decoder/encoder are a bit slower because they perform extra validation work.
...
However doing this work enables adding customization, for example, deduplicating the data using a process known as [string interning][interning-wiki].
