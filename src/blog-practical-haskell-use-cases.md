...
This post presents a few practical project in which we used Haskell succesfully.
...
Our goal was to perform Bugzilla API responses data processing.
...
Inspired by the work of Gabriel Gonzalez on interpreter, we used [megaparsec][megaparsec] to implement the language:
...
Similar to Servant, using Morpheus GraphQL adds strong garantees to our code.
...
In particular, Haskell helps us break monolith program into well defined and re-usable functions.
Being able to move the code fearlessly is incredibly powerfull.

Moreover, the Haskell community is constently producing interesting work.
