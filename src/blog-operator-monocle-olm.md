...
- both package managers rely on a manifest to ensure proper installation and configuration of the package (the operator).
...
- **operators**: It is the placeholder where one can subscribe to one or more operators.
...
From here, we will be able to extend our k8s by installing new operators.
...
- or we distribute the bundle via an existing catalog like [operatorhub-io].

The next section explains the process we followed to publish the Monocle Operator on `operatorbub.io`.
