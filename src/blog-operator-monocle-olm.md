...
First, we ensured that the **bundle metadata are accurate** (see the [required fields][required-fields]). To do so the CSV template needs to be adapted in `config/manifests/bases/monocle-operator.clusterserviceversion.yaml`.
