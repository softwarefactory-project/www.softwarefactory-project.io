...
Furthermore various attributes can be configured into the `spec` (see `api/v1alpha1/monocle_types.go`)
...
The `secret` is identified by its name and as a good practice Resources' names must be
...
Accessing the Monocle WEB UI access served by the API can be done using a `port-forward`:
...
As described previously we need to handle:
...
then `rollOutWhenApiSecretsChange` function checks if the resources version changed. In the case of a change
(meaning that the administrator changed one of the Monocle secrets) we do an `Update` of the annotation
to store the new `apiSecretsVersion` value.

This has the effect of triggering the Deployments rollout and get `Pods` to be restarted.
