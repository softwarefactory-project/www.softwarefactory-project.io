...
Capabilities of an operator are splitted into [phases][operator-phases] as described in the
...
is able to detect changes in its configuration file and reload itself the configuration.
...
according to the state. We use the [Client][controller-runtime-client] exposed through the `MonocleReconcilier` interface to perform CRUD actions.
...
It was a great and  chanllenging oportunity to learn this big new world of Kubernetes, and we hope to have made the right choices for the continuity of Monocle Operator.

Stay tuned for the next posts, where we will continue in this exiting new world of Operators.
