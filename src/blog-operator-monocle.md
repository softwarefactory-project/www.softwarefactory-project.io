This post aims to explain how we built a k8s Operator using the [operator sdk][operator sdk] for
the [Monocle][Monocle] project.
...
The `secret` is identified by its name and in general Resources' names must be
...
At `Deployment` creation we set an annotation called: `apiSecretsVersion`, and every time the `Reconcile` is called
...
- the ClusterRole `monocle-operator-manager-role`. This role defines [authorisations][k8s-rbac] needed by the
  `controller` to act on cluster's API. The authorisations are handled by the operator SDK through the
...
- the ClusterRoleBinding `monocle-operator-manager-rolebinding` that allows the `monocle-operator-controller-manager`
  Service Account to acts upon the resources.
...
A Monocle instance can be reclaim to the operator by applying the `Sample` resource:
...
relying on the [kustomize][kustomize] tool.
