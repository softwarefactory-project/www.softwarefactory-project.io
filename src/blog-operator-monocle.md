...
API and Crawler process are handled by the [Deployment Resource][k8s-deployment]. This resource's
controller handles a [rollout][k8s-deployment-rollout] workflow when the container's `Image` of
the `podSpec` or the `podTemplateSpec`'s annotations are updated. A `rollout` restarts `Pods` in
safe manner according to the configured rollout strategy.

To leverage that specificity of the `Deployment` Resource and then ensuring that API and Crawlers
containers are restarted when the Monocle's administrator change we made the following changes
(we only focus on the API, same apply for the Crawler's Deployment):
...
[k8s-core-jobs]: https://pkg.go.dev/k8s.io/api/core/v1#Jobs
